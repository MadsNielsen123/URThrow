...
//    std::cout << std::endl;
    ...
    std::vector<double> tSpeed = {0,0,0,0,0,0};
    std::vector<std::vector<double>> targetSpeed;
    std::vector<std::vector<double>> actualSpeeds;
    std::vector<double> actualTimes;
    std::vector<double> speedFrac;
    ...
    //Send first target speed
    for(int i= 0; i<6; ++i)
    {
            double tn = t+interval.count(); //t-next
    ...
    while(true)
    {
        ...
        if(currentTime - lastCommandTime >= interval)
        {
            for(int i= 0; i<6; ++i)
            {
                    double tn = t+interval.count(); //t-next
                    jointVelocity[i] = 4*a[i]*(tn*tn*tn) + 3*b[i]*(tn*tn)+ 2*c[i]*tn + d[i];
                    tSpeed[i] = 4*a[i]*(t*t*t) + 3*b[i]*(t*t)+ 2*c[i]*t + d[i];
            }
...
//    mGripperSocket.readAll(); //Empty buffer
    ...
    std::ofstream file("speedJAnalysis.csv");
    if (!file.is_open()) {
    ...
    // Write headers
    file << "targetSpeed,actualSpeeds,actualTimes,speedFrac\n";

    // Assuming all vectors are filled with equal sizes
    size_t rows = targetSpeed.size();
    for (size_t i = 0; i < rows; ++i) {
        // Write targetSpeed (comma-separated)
        for (size_t j = 0; j < targetSpeed[i].size(); ++j) {
            file << targetSpeed[i][j];
            if (j < targetSpeed[i].size() - 1) file << ";"; // Semi-colon for internal separation
        }
        file << ",";

        // Write actualSpeeds (comma-separated)
        for (size_t j = 0; j < actualSpeeds[i].size(); ++j) {
            file << actualSpeeds[i][j];
            if (j < actualSpeeds[i].size() - 1) file << ";"; // Semi-colon for internal separation
        }
        file << ",";

        // Write actualTimes and speedFrac
        file << actualTimes[i] << "," << speedFrac[i] << "\n";
    }

    file.close();
    std::cout << "Data successfully written to " << "speedJAnalysis.csv" << std::endl;
